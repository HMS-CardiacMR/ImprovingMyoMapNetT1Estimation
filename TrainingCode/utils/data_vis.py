...
def ntensorshow(x, sl_dims=(0, 0), rng=[0, 0.001], titles=None, saveFigs=False,figname=None):
    ...
    for sl in range(0,n_slices):
        ...
        for ax in axs:
            ...
            ax.imshow(img.cpu().data.numpy(), cmap='jet', vmin=0, vmax=2500)
...
def exportPNG(x, sl_dims=(0, 0), rng=[0, 0.001], titles=None, saveFigs=False, figname=None):
    ...
    for sl in range(0, n_slices):
        ...
        if saveFigs:
            fig.savefig(params.validation_dir + figname[sl] + '.png', dpi=300)
            plt.close(fig)